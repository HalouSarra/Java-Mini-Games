...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void paintComponent(Graphics g){
        super.paintComponent(g); //wipes the canvas clean like an eraser so you can draw the new frame
        draw(g);
    }
...
//            }
    ...
    public void checkCollisions(){
        //chack if head collies with body
        for (int i = bodyParts; i > 0; i--){
            if ((x[0] == x[i]) && (y[0] == y[i])){
                running = false;
            }
        }
        //chack if head touches the Left bound
        if (x[0] < 0){
            running = false;
        }
        //chack if head touches the Right bound
        if (x[0] > SCREEN_WIDTH){
            running = false;
        }
        //chack if head touches the Upper bound
        if (y[0] < 0){
            running = false;
        }
        //chack if head touches the Bottom bound
